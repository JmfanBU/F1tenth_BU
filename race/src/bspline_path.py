...
def bspline_planning(x, y, sn):
    ...
    ry_pre = ry[:-1]
    ry_late = ry[1:]
    rx_diff = rx_late - rx_pre
    ry_diff = ry_late - ry_pre
    ryaw = np.arctan2(ry_diff, rx_diff)
    rk = np.zeros(ryaw.shape)
    return rx[:-1], ry[:-1], ryaw, rk
...
def main():
    ...
    rx, ry, ryaw, rk = bspline_planning(x, y, sn)
